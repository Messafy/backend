...
@Repository
public class MongoPinNoteRepository implements PinNoteRepository {
    private final NoteMapper mapper;
    private final MongoDataNoteRepository repository;

    public MongoPinNoteRepository(NoteMapper mapper, MongoDataNoteRepository repository) {
        this.mapper = mapper;
    ...
    public Optional<Note> findById(NoteId id) {
        return repository.findById(mapper.toObjectId(id.id())).map(mapper::toDomain);
    }
